...
定义四元数的模长(标准名称为**范数**)，我们使用 $||p||$ 表示，有 $||p||=s^2+\vec{v}\cdot \vec{v}$ ，可以理解为四维上坐标到原点的距离。另外，可以证明 $||pq||=||p||||q||$

定义四元数 $p=s+\vec p$ 的共轭 $\hat{p}=s-\vec p$ ，发现 $p\hat p=s^2+\vec p\cdot\vec p=||p||$ ，所以有 $p^{-1}=\frac{\hat p}{||p||}$ 。在 $||p||=1$ 的情况下，我们有 $p^{-1}=\hat p$
